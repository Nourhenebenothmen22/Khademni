...
- **Mechanism**: Extracts the requested organization ID from `req.params.organizationId`, `req.query.organizationId`, or `req.headers['x-organization-id']`. Compares it against `req.user.organizationId`. Returns 403 Forbidden if a cross-tenant access attempt is detected.
- **Purpose**: Enforces tenant isolation for organization-scoped endpoints, preventing authenticated users from accessing resources belonging to other organizations.

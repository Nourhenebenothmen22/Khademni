...
function MfaLoginContent({ searchParams }: { searchParams: Promise<{ userId?: string; mfaToken?: string; redirect?: string }> }) {
  const router = useRouter();
  const { userId, mfaToken, redirect: redirectParam } = use(searchParams);
...
export default function MfaLoginPage({ searchParams }: { searchParams: Promise<{ userId?: string; mfaToken?: string; redirect?: string }> }) {
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col">
      ...
      <main className="flex-1 flex items-center justify-center p-4">
        <Suspense fallback={<div className="h-96 w-full max-w-md bg-white rounded-2xl animate-pulse" />}>
          <MfaLoginContent searchParams={searchParams} />
        </Suspense>
      </main>
    </div>
  );
